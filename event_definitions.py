"""
This module contains the logic for recording messages and other events for the NLP project.

Note that this module is usually part of a larger codebase, but copied here to preserve anonymity.
"""
import datetime
import logging
import re
...
class NLPRecorder:
    ...
    async def get_recording_channels(self, guild_id: int):
        ...
        """
        channels = []
        async for key in self.bot.rdb.iscan(match=f"cog.initiative.nlp.{guild_id}.*"):
            channel_id_match = re.match(r"cog\.initiative\.nlp\.(\d+)\.(\d+)\.recorded_combat_id", key)
    ...
    async def stop_all_recordings(self, guild: disnake.Guild):
        ...
        keys_to_delete = {key async for key in self.bot.rdb.iscan(match=f"cog.initiative.nlp.{guild_id}.*")}
        if keys_to_delete:
    ...
    async def _recording_info(self, guild_id: int, channel_id: int) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # get from redis
            combat_id = await self.bot.rdb.get(f"cog.initiative.nlp.{guild_id}.{channel_id}.recorded_combat_id")
            # and write to memory cache
            if combat_id is None:
                channel_recording_until = 0
            else:
                channel_recording_ttl = await self.bot.rdb.ttl(
                    f"cog.initiative.nlp.{guild_id}.{channel_id}.recorded_combat_id"
                )
